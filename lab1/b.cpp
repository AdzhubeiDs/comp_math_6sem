...
void plotGraphForC(ld c, ld h, const vector<vector<ld>>& u, const vector<ld>& x) {
    string path = "graphs_b";
    system(("mkdir -p " + path).c_str());

    vector<ld> tList = {0,1, 2, 3, 4, 5, 7, 9, 10, 11, 13, 15, 17, 18};
...
void solveForC(ll T, ll L, ll NX, ld c, ld h) {
    ...
    for (size_t n = 0; n < NT - 1; n++) {
        for (size_t m = 1; m < NX-1 ; m++) { // ??? мб надо заменить на 1 т.к. m=0 не будет никогда 
            u[m][n + 1] = 
                (pow(c, 2) / 2) *
                (u[m + 1][n] - 2 * u[m][n] + u[m - 1][n]) - (c / 2) *
                (u[m + 1][n] - u[m - 1][n]) + u[m][n];
        }
        u[NX - 1][n + 1] =
            (pow(c, 2) / 2) *
            (u[0][n] - 2 * u[NX - 1][n] + u[NX - 2][n]) - (c / 2) *
            (u[0][n] - u[NX - 2][n]) + u[NX - 1][n];
